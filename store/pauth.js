...
const getters = {
    ...
    getPromptMsg: state => state.promptMsg,
...
const actions = {
    ...
    fetchToken({commit, dispatch}, data) {
        authService.fetchToken(data).then(function (resp) {
            if (resp.data != null && resp.data.code != null && resp.data.code >=0) {
                ...
                dispatch("displayPromptByResponse", resp);
            }
        }.bind(this)).catch(function (err) {
            commit("refreshInvalidLoginState", true);
            dispatch("displayPromptByResponse", err.response);
        }.bind(this));
    ...
    refreshToken({commit, dispatch}) {
        ...
        if (refreshToken != null) {
            authService.refreshToken({'refresh_token': refreshToken}).then(function (resp) {
                if (resp.data != null && resp.data.code != null && resp.data.code >= 0) {
                    jwtTokenUtil.saveAccess(resp.data.details.accessToken);
                    dispatch("readLoginInfo");
                }
    ...
    revokeToken({dispatch}) {
        ...
        if (accessToken != null) {
            authService.revokeToken({token: accessToken}).then(function (resp) {
                dispatch("clearToken");
            }.bind(this)).catch(function (err) {
                dispatch("displayPromptByResponse", err.response);
            }.bind(this));
     ...
     */
    login({dispatch}) {
        authService.fetchLoginUrl().then(function (resp) {
            if (resp.data != null && resp.data.code != null && resp.data.code >= 0) {
	...
	/**
     * 从localstorage读取最近一次用户访问的地址并更新到state，用于登录后跳转
     * @param commit    store state更新提交者
     */
     ...
     */
    displayPromptByResponse({commit}, response) {
        if (response != null && response.status != null && response.status == 200) {
            commit("refreshPromptMsg", {code: response.data.code, details: response.data.details});
        } else {
            let errorMsg = "请求失败，";
            if (response == null) {
                errorMsg += "访问后端服务返回异常。";
            ...
            commit("refreshPromptMsg", {code: -1, details: errorMsg});
        }
    }
};
...
const mutations = {
    ...
    refreshPromptMsg(state, data) {
        state.promptMsg = data;
    },
